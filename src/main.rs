...
#[derive(Debug, Clone)]
enum Message {
    PokémonFound(Result<Pokémon, Error>),
    Search,
}

impl Application for Pokédex {
    type Executor = iced::executor::Default;
    type Message = Message;
    type Flags = ();

    fn new(_flags: ()) -> (Pokédex, Command<Message>) {
        (
            Pokédex::Loading,
            Command::perform(Pokémon::search(), Message::PokémonFound),
        )
    }

    fn title(&self) -> String {
        let subtitle = match self {
            Pokédex::Loading => "Loading",
            Pokédex::Loaded { pokemon, .. } => &pokemon.name,
    ...
    fn update(&mut self, message: Message) -> Command<Message> {
        match message {
            ...
            Message::Search => match self {
                Pokédex::Loading => Command::none(),
                _ => {
                    *self = Pokédex::Loading;
...
impl Pokémon {
    ...
    async fn search() -> Result<Pokémon, Error> {
        use rand::Rng;
